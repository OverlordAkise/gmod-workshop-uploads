...
--Find more of my free addons on: https://github.com/OverlordAkise/darkrp-addons

--Changes to non-workshop version:
-- Uses console variables for configuration

local ScoreFrame = nil
local Inspect = nil
local IsClosing = nil

--Config is via console variables, check the sh file
...
local function CreateScoreboard()
    ...
    ScoreFrame.Paint = function( self, w, h )
        ...
        draw.DrawText(LUCTUS_SCOREBOARD_CONVAR_TITLE:GetString(), "LuctusScoreFontBig", w / 2, 5, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER)
    ...
    local offsetX, offsetY = surface.GetTextSize(LUCTUS_SCOREBOARD_CONVAR_URL:GetString())
    ...
    website:SetText(LUCTUS_SCOREBOARD_CONVAR_URL:GetString())
    website:SetCursor("hand")
    website:SetMouseInputEnabled( true )
    website.OnMousePressed = function()
        gui.OpenURL(LUCTUS_SCOREBOARD_CONVAR_URL:GetString())
    end
